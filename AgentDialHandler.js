...
function saveContactBulk(req, jobId) {
  ...
  var jsonString;
  DbConn.DialerAgentDialInfo.bulkCreate(dialerAgentDialInfo, {
    validate: false,
    ...
    .then(function (results) {
      ...
      logger.info(
        "[Agent-Dial-handler.SaveDialInfo] - [PGSQL] - SaveContacts successfully.[%s] ",
        jsonString
      );
    })
    .catch(function (err) {
      ...
      logger.error(
        "[Agent-Dial-handler.SaveDialInfo] - [%s] - [PGSQL] - SaveContacts failed",
        req.user.company,
        err
      );
    })
  ...
  while (tempData.length) {
    ...
    agentNumberList[agent.displayName] = {
      ...
      Data: tempData.splice(0, chunk).map(function (item) {
        return {
          Number: item[numberColumnName.toString()],
          OtherData: item[dataColumnName.toString()],
        };
      }),
    };
    i++;
...
var addToHistory = function (item) {
  ...
  DbConn.DialerAgentDialInfoHistory.create({
    ...
    .then(function (results) {
      ...
      logger.info(
        "addToHistory - [PGSQL] - Updated successfully.[%s] ",
        jsonString
      );
    })
    .error(function (err) {
      ...
      logger.error(
        "addToHistory - [%s] - [PGSQL] - UpdateDialInfo failed-[%s]",
        item.AgentDialNumberId,
        err
      );
    });
};
...
module.exports.UpdateDialInfo = function (req, res) {
  ...
  if (!req.user || !req.user.tenant || !req.user.company) {
    jsonString = messageFormatter.FormatMessage(
      new Error("invalid tenant or company."),
      "EXCEPTION",
      false,
      null
    );
    ...
    DbConn.DialerAgentDialInfo.find({
      where: [
        { AgentDialNumberId: dialId },
        { TenantId: req.user.tenant },
        { CompanyId: req.user.company },
      ],
    })
      .then(function (cmp) {
        if (cmp) {
          ...
          DbConn.DialerAgentDialInfo.update(
            {
            ...
            .then(function (results) {
              ...
              logger.info(
                "UpdateDialInfo - [PGSQL] - Updated successfully.[%s] ",
                jsonString
              );
              res.end(jsonString);
            })
            .error(function (err) {
              ...
              logger.error(
                "UpdateDialInfo - [%s] - [PGSQL] - UpdateDialInfo failed-[%s]",
                dialId,
                err
              );
              res.end(jsonString);
      ...
      .error(function (err) {
        logger.error(
          "UpdateDialInfo - [%s] - [PGSQL] - UpdateDialInfo  failed",
          dialId,
          err
        );
        jsonString = messageFormatter.FormatMessage(
  ...
  function getNumbers() {
    ...
    var query = {
      ...
      where: [
        {
          StartDate: {
            $lte: moment.utc(req.params.StartDate).format("YYYY-MM-DD"),
          },
        },
        { ResourceId: req.params.ResourceId },
        { TenantId: req.user.tenant },
    ...
    DbConn.DialerAgentDialInfo.findAll(query)
      .then(function (cmp) {
        if (cmp && cmp.length != 0) {
          jsonString = messageFormatter.FormatMessage(
            null,
  ...
  var querys = [
    {
      attributes: [
        [
          DbConn.SequelizeConn.fn(
            "DISTINCT",
            DbConn.SequelizeConn.col("BatchName")
          ),
          "BatchName",
        ],
      ],
    ...
    },
    {
      attributes: [
        [
          DbConn.SequelizeConn.fn(
            "DISTINCT",
            DbConn.SequelizeConn.col("DialerState")
          ),
          "DialerState",
        ],
      ],
...
module.exports.agentDialerDispositionSummaryReportCount = function (req, res) {
  ...
  DbConn.DialerAgentDialInfo.count(query)
...
var extractResourceWiseSummary = function (
  batchName,
  startDate,
  endDate,
  companyId,
  ...
  try {
    ...
    DbConn.DialerAgentDialInfo.aggregate("*", "count", query)
      .then(function (tryCount) {
        var obj = {
...
module.exports.agentDialerAgentSummaryReport = function (req, res) {
  ...
  DbConn.DialerAgentDialInfo.aggregate("ResourceId", "DISTINCT", query)
    .then(function (distinctResources) {
      if (distinctResources) {
        var arr = [];
        distinctResources.forEach(function (resource) {
          arr.push(
            extractResourceWiseSummary.bind(
              this,
              batchName,
              startDate,
              endDate,
...
module.exports.agentDialerDispositionDetailsReportCount = function (req, res) {
  ...
  DbConn.DialerAgentDialInfoHistory.count(query)
    .then(function (CamObject) {
      if (CamObject) {
        jsonString = messageFormatter.FormatMessage(
          null,
...
module.exports.agentDialerDispositionDetailsReport = function (req, res) {
  ...
  DbConn.DialerAgentDialInfoHistory.findAll(query)
    .then(function (CamObject) {
      if (CamObject) {
        jsonString = messageFormatter.FormatMessage(
          null,
